/**
 * @file Génération du PDF — version NATIVE (iOS / Android)
 * Sur web, Metro charge exportPDF.web.ts à la place de ce fichier.
 */
...
import { buildMissionHTML } from '@/utils/missionPDFTemplate';
...
/**
 * Construit l'en-tête HTML du document PDF.
 */
...
/**
 * Construit le tableau de décomposition HTML.
 */
...
/**
 * Construit le bloc récapitulatif HTML.
 */
...
/**
 * Construit le CSS intégré du document PDF.
 */
 ...
 *
 * @param devis   - Données du devis
 * @param lignes  - Lignes de décomposition calculées
 * @param recap   - Récapitulatif calculé
 * @returns Chaîne HTML prête pour expo-print
 */
...
/**
 * Exporte le devis en PDF et propose le partage.
 * Nécessite : npx expo install expo-print expo-sharing
 *
 * @param devis  - Données du devis
 * @param lignes - Lignes calculées
 * @param recap  - Récapitulatif calculé
 */
/**
 * Exporte le devis (ancien format) en PDF et propose le partage.
 */
export async function exporterPDF(
  ...
  try {
    const html = genererHTMLDevis(devis, lignes, recap);
    ...
    if (isAvailable) {
      await Sharing.shareAsync(uri, {
        ...
        dialogTitle: `Devis ${devis.clientInfo.reference}`,
        UTI: 'com.adobe.pdf',
      });
    } else {
    ...
    throw new Error(`Échec de l'export PDF : ${message}`);
  }
}

/**
 * Exporte un bon de commande mission en PDF (format NFS de référence).
 */
export async function exporterMissionPDF(mission: DevisMission): Promise<void> {
  try {
    const html = buildMissionHTML(mission);
    const { uri } = await Print.printToFileAsync({ html, base64: false });
    ...
    if (isAvailable) {
      await Sharing.shareAsync(uri, {
        mimeType: 'application/pdf',
        dialogTitle: `Bon de commande ${mission.mission.numero_devis}`,
        UTI: 'com.adobe.pdf',
      });
    } else {
      throw new Error("Le partage de fichiers n'est pas disponible sur cet appareil");
    ...
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    throw new Error(`Échec de l'export PDF mission : ${message}`);
  }
}
